...
// Set Variables
$LOCAL_ROOT         = "/var/www/qian";
$LOCAL_REPO_NAME    = "";
$LOCAL_REPO         = "{$LOCAL_ROOT}/{$LOCAL_REPO_NAME}";
$REMOTE_REPO        = "git@github.com:sbourne20/qian.git";
$BRANCH             = "master";
...
if ( $_POST['payload'] ) {
  ...
  if( file_exists($LOCAL_REPO) ) {
    ...
    shell_exec("sudo su apache && cd {$LOCAL_REPO} && git pull");

    die("done " . mktime());
  } else {

    // If the repo does not exist, then clone it into the parent directory
    shell_exec("cd {$LOCAL_ROOT} && sudo su apache && git clone {$REMOTE_REPO}");
